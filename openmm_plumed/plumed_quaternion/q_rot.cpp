...
void Quaternion::optimalAlignment1(const std::vector<Vector> & currpos)
{
    ...
    */
    ...
    if (!bRefc2_is_calculated) {
        ...
        //fprintf(stderr, "2 groups Domain1");
        for (unsigned ia=0; ia<refpos2.size(); ia++) {
            if (refw2[ia]==0) continue; //Only apply forces to weighted atoms.

            double const rx = refpos2[ia][0];
            double const ry = refpos2[ia][1];
            double const rz = refpos2[ia][2];
            ...
            for (unsigned p=0; p<4; p++) {
                ...
                for (unsigned i=0 ;i<4; i++) {
                    for (unsigned j=0; j<4; j++) {
                        dqidxj += -1 * (
                                (qmat[1][i]*qmat[0][j]*dSdx[i][j])/(eigenvals[0]-eigenvals[1])*qmat[1][p] +
                                (qmat[2][i]*qmat[0][j]*dSdx[i][j])/(eigenvals[0]-eigenvals[2])*qmat[2][p] +
                                (qmat[3][i]*qmat[0][j]*dSdx[i][j])/(eigenvals[0]-eigenvals[3])*qmat[3][p]);
                ...
                //fprintf(stderr, "Domain 2 %g %g %g\n", dqidxj[0], dqidxj[1], dqidxj[2]);
                setAtomsDerivatives(qptr[p], ia+offset, dqidxj);
            }
        }
        ...
        for (unsigned ia=0; ia<rotref1.size(); ia++) {
            ...
            double const rx = rotref1[ia][0];
            double const ry = rotref1[ia][1];
            double const rz = rotref1[ia][2];
