...
# coords = pmd.namd.namdbinfiles.NamdBinCoor.read('../000_eq/outputs_namd/eq.coor').coordinates[0]
...
traj_file = osp.realpath("/home/ndonyapour/projects/BFEE2/NAMD_bfee2_2wi3/BFEE_NAMD/001_RMSDBound/output/abf_1.dcd")
#traj_file = osp.realpath ('inputs/eq.dcd')
coords = mdj.load_dcd(traj_file, 'inputs/complex_namd.pdb').xyz
...
pe = []
for idx, frame_coords in enumerate(coords):
    simulation.context.setPositions(frame_coords)
    pe_decompose = pmd.openmm.energy_decomposition(prmtop, simulation.context, nrg=unit.kilojoules_per_mole)
    pe.append(list(pe_decompose.values()))

np.save("inputs/pe_rmsdbound.npy", np.array(pe))